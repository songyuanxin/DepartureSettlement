...
public class ManagerController {
    ...
    public AjaxResult regionalNoAdopt(RegionalOrAreaApproveDto regionalOrAreaApproveDto){
        ...
        //为ImportDataDto实体赋值
        BeanUtils.copyProperties(importData, importDataDto);
        importDataDto.setPernr(importData.getQuitPernr());
        importDataDto.setName(userName);

        List<ImportDataDto> importDataDtoList = new ArrayList<>();
        importDataDtoList.add(importDataDto);

        if (importDataDtoList.size() == 0){
            return AjaxResult.error("找不到该员工直接上级等信息，请联系管理员！");
        }
        //2、调用发送至直接上级企业微信接口向直接上级重新推送该员工的离司结算申请
        String isReturn = "1";
        SendMsgRes sendMsgRes = importDataService.launchProcess(importDataDtoList, isReturn);
        if (sendMsgRes.getErrcode() != 0){
